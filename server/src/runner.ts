...
export class IntervalRunner implements Runner {
  ...
  play() {
    ...
    this._timer = setInterval(() => this.onTick(this.fps), 50);
    return this;
  }
}

// Author: https://github.com/ZeroTixDev/accurate-game-loop
class Loop {
  ...
  start(): Loop {
    ...
    function _tick(): any {
      ...
      if (
        _this._ConvertNanoToMs(Math.abs(expectedLength - averageDelta)) >=
        jitterThreshold
      ) {
...
export class ElasticRunner implements Runner {
  public readonly onTick = signal<number>();

  public readonly loop: Loop;

  constructor(public readonly fps: number) {
    this.loop = new Loop(this.onTick, fps);
  }
